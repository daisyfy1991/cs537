...
//        cprintf("before reading, dst: %s\n", dst);
//        cprintf("going to read into inode: %s\n", (char *)ip->addrs);
//        cprintf("off: %d, n:%d filesize: %d\n", off, n, ip->size);
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  if(off > ip->size || off + n < off) //n: number of bytes wish to write
    return -1;
...
//        cprintf("now in node: %s\n", (char *)ip->addrs);
  ...
  // make sure for small file that inode is updated
  return n;
}
